const API_KEY = 'es1BYTcrl0Y2Lfyxatmig9d4YHoS9IyoqYptIQZC'
const API_URL = `https://api.nasa.gov/insight_weather/?api_key=${API_KEY}&feedtype=json&ver=1.0`

const pretoglle = document.querySelector('.show-previous-weather')
const preweat = document.querySelector('.previous-weather')
...
pretoglle.addEventListener('click', ()=>{
    preweat.classList.toggle('show-weather')
})
...
function displaySelectedSol(sols) {
    ...
    windSpeedElement.innerText = displayWind(selectedSol.windSpeed)
    windDirectionArrow.style.setProperty('--direction', `${selectedSol.windDirectionDegrees}deg`)
    windDirectionText.innerText = selectedSol.windDirestionCardinal
        ...
        .then(data => {
         ...
         return Object.entries(solData).map(([sol, data]) => {
             return {
                ...
                windDirestionCardinal: data.WD.most_common.compass_point,
